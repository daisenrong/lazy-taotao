(function ($) {
    $.learuntab = {
        ...
        addTab: function () {
            ...
            if ($(".navbar-custom-menu>ul>li").length >= 30) {
                layer.alert("已经开启了30个标签页了，太多了，请关闭几个不需要的吧！！");
                return false;
            }
            ...
            //拦截  打开url为空的菜单
            if (dataUrl == undefined || $.trim(dataUrl) == ctx) {
                layer.msg("菜单URL为空，无法打开");
                return false;
            }
                        ...
                        $.each(childNodes, function (i) {
                            ...
                            if (subchildNodes.length > 0) {
                                ...
                                $.each(subchildNodes, function (i) {
                                    ...
                                    _html += '<li><a class="menuItem" data-id="' + subchildNodesrow.id + '" href="' + ctx + subchildNodesrow.urlAddress + '"><i class="' + subchildNodesrow.icon + '"></i>' + subchildNodesrow.name + '</a></li>';
                                });
                                _html += '</ul>';

                            } else {
                                _html += '<a class="menuItem" data-id="' + subrow.id + '" href="' + ctx + subrow.urlAddress + '"><i class="' + subrow.icon + '"></i>' + subrow.name + '</a>';
                            }
                            _html += '</li>';
                        });
                        _html += '</ul>';
    ...
    $(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.learunindex.loadMenu(data.data);
                $.learuntab.init();
            }
        });
